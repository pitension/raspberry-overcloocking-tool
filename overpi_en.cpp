...
: m_mainBox(Gtk::ORIENTATION_VERTICAL, 5),
  ...
  m_threadRunning(true) {
    ...
    // Configure controls
    m_profileCombo.append("Minimum");
    m_profileCombo.append("Normal");
    m_profileCombo.append("Moderate");
    m_profileCombo.append("High");
    m_profileCombo.append("Extreme");
    m_profileCombo.set_active_text("Normal");
    m_currentProfile = "Normal";
    
    m_applyHotBtn.set_label("Apply Hot");
    m_applyPermBtn.set_label("Apply Permanently");
    m_infoBtn.set_label("System Info");
    
    m_metricsFrame.set_label("System Status");
    m_descFrame.set_label("Profile Description");
    
    m_warningLabel.set_label("⚠️ WARNING: EXTREME profiles can damage your hardware. Require active cooling.");
...
void PiOverclockApp::loadProfiles() {
    ...
    minimum.desc = "Minimum operation configuration\n- CPU: 600 MHz\n- GPU: 500 MHz\n- Overvoltage: 0\n- Force Turbo: 0";
    minimum.color = "#27ae60";
    m_profiles["Minimum"] = minimum;
    ...
    normal.desc = "Factory default configuration\n- CPU: 1800 MHz\n- GPU: 500 MHz\n- Overvoltage: 0\n- Force Turbo: 0";
    normal.color = "#27ae60";
    ...
    moderate.desc = "Slight performance increase\n- CPU: 1900 MHz\n- GPU: 550 MHz\n- Overvoltage: 2\n- Force Turbo: 0";
    moderate.color = "#3498db";
    m_profiles["Moderate"] = moderate;
    ...
    high.force_turbo = "0";
    high.temp_limit = "70";
    high.desc = "Improved performance\n- CPU: 2000 MHz\n- GPU: 600 MHz\n- Overvoltage: 4\n- Force Turbo: 0";
    high.color = "#f39c12";
    m_profiles["High"] = high;
    ...
    extreme.desc = "Maximum performance (dangerous)\n- CPU: 2200 MHz\n- GPU: 750 MHz\n- Overvoltage: 8\n- Force Turbo: 0\n\n⚠️ REQUIRES GOOD COOLING";
    extreme.color = "#e74c3c";
    m_profiles["Extreme"] = extreme;
}
...
void PiOverclockApp::onProfileChanged() {
    ...
    if (m_profiles.find(m_currentProfile) != m_profiles.end()) {
        ...
        m_statusLabel.set_label("Current profile: " + m_currentProfile);
    }
}
...
void PiOverclockApp::onApplyHotClicked() {
    ...
    if (selectedProfile == "Extreme") {
        if (!showQuestionDialog("⚠️ DANGER - EXTREME OVERCLOCK", 
            "EXTREME PROFILE IS DANGEROUS:\n\n"
    ...
    if (showQuestionDialog("Apply Hot", 
        "Profile '" + selectedProfile + "' will be applied hot.\n\n"
        "⚠️ LIMITATIONS:\n"
...
void PiOverclockApp::onApplyPermClicked() {
    ...
    if (showQuestionDialog("Confirm Permanent Change",
        "Are you sure to apply profile '" + selectedProfile + "' permanently?\n\n"
...
void PiOverclockApp::onInfoClicked() {
    ...
    try {
        ...
        if (!tempStr.empty()) {
            ...
            info += "Temperature: " + std::to_string(temp) + " °C\n";
        }
    } catch (...) {
...
void PiOverclockApp::updateSystemInfo() {
    ...
    try {
        ...
        if (!tempStr.empty()) {
            float temp = std::stof(tempStr) / 1000.0;
            m_cpuTempLabel.set_label("CPU Temperature: " + std::to_string(temp) + " °C");
        }
    } catch (...) {
        m_cpuTempLabel.set_label("CPU Temperature: N/A");
    }
    ...
    try {
        ...
        if (!freqStr.empty()) {
            ...
            m_cpuFreqLabel.set_label("CPU Frequency: " + std::to_string(freq) + " MHz");
        }
    } catch (...) {
        m_cpuFreqLabel.set_label("CPU Frequency: N/A");
    }
    ...
    if (!gpuFreq.empty() && gpuFreq.find("=") != std::string::npos) {
        try {
            ...
            m_gpuFreqLabel.set_label("GPU Frequency: " + std::to_string(freq) + " MHz");
        } catch (...) {
            m_gpuFreqLabel.set_label("GPU Frequency: N/A");
        }
    } else {
        m_gpuFreqLabel.set_label("GPU Frequency: N/A");
    }
    ...
    try {
        ...
        std::string governor;
        if (govFile) std::getline(govFile, governor);
        m_cpuGovLabel.set_label("CPU Governor: " + governor);
    } catch (...) {
        m_cpuGovLabel.set_label("CPU Governor: N/A");
    }
    
    // Update throttling information
    std::string throttlingInfo = getThrottlingInfo();
    m_throttlingStatusLabel.set_label("Status: " + throttlingInfo);
    m_throttlingDetailsLabel.set_label("Details: " + decodeThrottling(execCommand("vcgencmd get_throttled")));
}
...
void PiOverclockApp::applyHotProfile(const std::string& profileName, bool showMessage) {
    ...
    try {
        ...
        if (retFreq != 0) {
            showMessageDialog("Error", "Could not adjust CPU frequency.\nMake sure 'cpufrequtils' is installed and you have root permissions.", Gtk::MESSAGE_ERROR);
            return;
        }

        // Update interface status
        m_currentProfile = profileName;
        m_statusLabel.set_label("Current profile: " + profileName + " (Hot applied)");

        if (showMessage) {
            showMessageDialog("Partial Success",
                "Profile " + profileName + " partially applied hot.\n\n"
        ...
        showMessageDialog("Error", "An error occurred while applying the profile: " + std::string(e.what()), Gtk::MESSAGE_ERROR);
    }
}
...
void PiOverclockApp::applyPermanentProfile(const std::string& profileName) {
    ...
    try {
        ...
        if (showQuestionDialog("Success", 
            "Profile " + profileName + " applied permanently.\n\n"
        ...
        showMessageDialog("Error", "Could not modify config.txt: " + std::string(e.what()), Gtk::MESSAGE_ERROR);
    }
}
...
std::string PiOverclockApp::getThrottlingInfo() {
    ...
    if (result.empty() || result.find("=") == std::string::npos) {
        return "Error getting throttling";
    }
...
std::string PiOverclockApp::decodeThrottling(const std::string& throttledHex) {
    try {
        ...
        if (throttledCode & 0x2) messages.push_back("Frequency capped now");
        if (throttledCode & 0x4) messages.push_back("Throttling now");
        if (throttledCode & 0x8) messages.push_back("Temperature limit now");
        
        // Historical bits
        if (throttledCode & 0x10000) messages.push_back("Under-voltage occurred");
        if (throttledCode & 0x20000) messages.push_back("Frequency capped occurred");
        if (throttledCode & 0x40000) messages.push_back("Throttling occurred");
        if (throttledCode & 0x80000) messages.push_back("Temperature limit occurred");
        ...
        return "Hexadecimal value: " + throttledHex;
    }
}

std::string PiOverclockApp::execCommand(const std::string& cmd) {
    char buffer[128];   // classic C buffer
    std::string result;
...
int main(int argc, char *argv[]) {
    ...
    if (access("/sys/class/thermal/thermal_zone0/temp", F_OK) == -1) {
        std::cerr << "Error: This application only works on Raspberry Pi" << std::endl;
        return 1;
    }
    
    // Check superuser permissions
    if (geteuid() != 0) {
        std::cerr << "Error: Run with: sudo " << argv[0] << std::endl;
        return 1;
    }
